...
export async function checkout(
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3001"}/table?session_id={CHECKOUT_SESSION_ID}&payment_method=promptpay&bill_id=${billId}`,
      cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3001"}/table?cancel=true&payment_method=promptpay&bill_id=${billId}`,
    });
